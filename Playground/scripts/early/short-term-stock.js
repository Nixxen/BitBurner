...
export async function main(ns) {
    ...
    let moneyKeep = 28 * 10 ** 9 // 1b - Adjust this lower if you do not have enough yet (or higher if you want more spare money)
    ...
    if (ns.fileExists(commissionFeeFile)){
        ...
        if (isNaN(parseInt(data))){
            ...
            ns.print(`Warning - Corrupt data in ${commissionFeeFile}. Deleting file as starting fresh.`);
        }else{
            cumulativeCommisionFee = parseInt(data);
            ns.print(`Info - Loaded previously saved commission data, ${ns.nFormat(cumulativeCommisionFee, formatString)}, from ${commissionFeeFile}. To start from zero, delete the file and restart the script.`);
        }
    }
    ...
    while (true) {
        ...
        let stocks = ns.stock.getSymbols().sort(function (a, b) { return ns.stock.getForecast(b) - ns.stock.getForecast(a); })
        ...
        if (!changes){
            ...
            let percentageMissing = (playerMoney - moneyKeep) / minBuyCost * 100
            if (percentageMissing < 100){
                ns.print(`Cycle complete, insufficient money to offset commision fee. Accumulating money: ${percentageMissing.toFixed(2)}% of ${ns.nFormat(minBuyCost, formatString)}`);
                tooMuchMoney = false;
            }else if (!tooMuchMoney){
                ns.print('Cycle complete, enough money, but no more viable stocks to buy. Squelching logs while awaiting stock changes...');
    ...
    async function buyPositions(stock) {
        ...
        if (forecast >= (stockBuyPer + volPer) && volPer <= stockVolPer) {
            let playerMoney = ns.getServerMoneyAvailable('home');
            let shares = Math.max(Math.min((playerMoney - moneyKeep - commissionFee) / askPrice, maxShares), minSharePerPurchase);
            let currentMinBuyPrice = ns.stock.getPurchaseCost(stock, shares, "Long");
            if (playerMoney - moneyKeep > currentMinBuyPrice && currentMinBuyPrice > minBuyCost) {
                changes = true;
    ...
    async function updateCommissionTotal() {
        ...
        ns.print(`Info - Total commission fees: ${ns.nFormat(cumulativeCommisionFee, formatString)}`);
    }
    ...
    async function sellPositions(stock) {
        ...
        if (forecast < (stockSellLimit + volPer)) {
            ...
            ns.tprint(`Sold: ${stock} with a profit of ${ns.nFormat(profit, formatString)} for a total return of ${ns.nFormat(returns, formatString)}`);
        }
        return changes;
    }
}